...
<?php 
    ...
    while($statusmodalrow = mysqli_fetch_assoc($statusmodalresult)){
        ...
        if ($status >= 1 && $status <= 4) {
            ...
            $deliveryDetailStmt = $conn->prepare($deliveryDetailSql);
            $deliveryDetailStmt->bind_param("i", $orderid);
            $deliveryDetailStmt->execute();
            $deliveryDetailResult = $deliveryDetailStmt->get_result();
            
            if ($deliveryDetailResult && $deliveryDetailRow = $deliveryDetailResult->fetch_assoc()) {
                $trackId = $deliveryDetailRow['id'];
                $deliveryBoyName = $deliveryDetailRow['deliveryBoyName'];
                $deliveryBoyPhoneNo = $deliveryDetailRow['deliveryBoyPhoneNo'];
                $deliveryTime = $deliveryDetailRow['deliveryTime'];
                ...
                // Handle the case where delivery details are not found
                $trackId = 'xxxxx';
                $deliveryBoyName = '';
                $deliveryBoyPhoneNo = '';
                $deliveryTime = 'xx';
